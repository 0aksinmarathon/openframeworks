...
void ofApp::setup(){
    ...
    generation = 0;
    rate = 1;
    std::vector<std::vector<bool>> tmp_false(num_vertical, std::vector<bool>(num_horizontal, false));
    state = tmp_false;
    reset();
    ofSetFrameRate(100);
...
void ofApp::mouseDragged(int x, int y, int button){
    try{
        ...
        if (coordinate_x < 0 || coordinate_x >= num_horizontal ||
            coordinate_y < 0 || coordinate_x >= num_vertical){
